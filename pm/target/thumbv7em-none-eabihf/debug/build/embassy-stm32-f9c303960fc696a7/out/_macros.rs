macro_rules ! set_clocks { ($ ($ (# [$ m : meta]) * $ k : ident : $ v : expr ,) *) => { { # [allow (unused)] struct Temp { $ ($ (# [$ m]) * $ k : Option < crate :: time :: Hertz > ,) * } let all = Temp { $ ($ (# [$ m]) * $ k : $ v ,) * } ; crate :: rcc :: set_freqs (crate :: rcc :: Clocks { afif : all . afif . into () , hclk1 : all . hclk1 . into () , hclk2 : all . hclk2 . into () , hclk3 : all . hclk3 . into () , hse : all . hse . into () , hsi : all . hsi . into () , hsi_hse : all . hsi_hse . into () , lse : all . lse . into () , lsi : all . lsi . into () , pclk1 : all . pclk1 . into () , pclk1_tim : all . pclk1_tim . into () , pclk2 : all . pclk2 . into () , pclk2_tim : all . pclk2_tim . into () , pll1_q : all . pll1_q . into () , plli2s1_p : all . plli2s1_p . into () , plli2s1_q : all . plli2s1_q . into () , plli2s1_r : all . plli2s1_r . into () , pllsai1_p : all . pllsai1_p . into () , pllsai1_q : all . pllsai1_q . into () , rtc : all . rtc . into () , sys : all . sys . into () , }) ; } } ; }
#[allow(unused)]
macro_rules! foreach_flash_region {
    ($($pat:tt => $code:tt;)*) => {
        ...
        __foreach_flash_region_inner!((Bank1Region1,16,16384));
        __foreach_flash_region_inner!((Bank1Region2,16,65536));
        __foreach_flash_region_inner!((Bank1Region3,16,131072));
        __foreach_flash_region_inner!((OTPRegion,16,0));
    };
}
#[allow(unused)]
