...
impl<'d, T: Instance> Bus<'d, T> {
    fn init(&mut self) {
        ...
        // STM32F723/F730/F733: OTGPHYC (InternalHighSpeed) also needs the ULPI AHB clock
        // even though it is an "internal" PHY — it connects to OTG_HS via the ULPI interface.
        let phy_type = self.inner.phy_type();
        let _ulpien = !phy_type.internal() || phy_type == PhyType::InternalHighSpeed;
